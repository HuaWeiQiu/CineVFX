...
| Photoshop UXP | 已完成开发预览 | `156/156` 测试；项目内 TypeScript、检查和构建通过 |
| Node 真实 socket 集成 | 已完成 | root `15/15`，其中 4 项是真实 loopback socket 测试 |
| 本地柔和发光 | macOS 实机通过 | Photoshop 2026 / 27.9.1 创建、撤销、重做成功 |
...
最后一次执行结果为 `VERIFY_EXIT=0`，包含全部 package/root 的
`check`、`test`、真实 TypeScript 编译和 `build`。
...
5. [`docs/agent-pack/02_SYSTEM_ARCHITECTURE.md`](agent-pack/02_SYSTEM_ARCHITECTURE.md)：总体架构。
...
- 写入后核对源层属性和派生图层结构。
- 成功 `resumeHistory(..., true)`；失败或取消 `resumeHistory(..., false)`。
...
1. 在 Windows Photoshop 2026 重复创建、撤销、重做。
2. 用透明人物像素层和智能对象各做一次真实验收。
